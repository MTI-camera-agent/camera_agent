...
CODE="$(curl -sS -o /tmp/camera_agent_minicpm_models.out -w "%{http_code}" "${BASE_URL}/v1/models" 2>/dev/null || true)"
echo "http_status=${CODE:-none}"
if [[ "${CODE}" == "200" ]]; then
  head -c 400 /tmp/camera_agent_minicpm_models.out 2>/dev/null || true
  echo
fi
rm -f /tmp/camera_agent_minicpm_models.out
