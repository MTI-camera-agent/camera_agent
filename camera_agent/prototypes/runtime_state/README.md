...
```text
i  set an intention
s  provide settled evidence and start orienting
 g return an Instruction
m  begin a material adjustment
s  settle and start evaluation
...
## Deliberately testable decisions
...
- **Ready is represented here as a new Instruction.** `Ready—take the shot.` receives its own Instruction identity and evidence-backed readiness record. Repeated confirmation preserves that identity. This is intentionally exposed for human challenge.
...
- **Async work is tokened.** Analysis completion must match run, task, and evidence identities. Generated visual output must additionally match its visual job and source Instruction. Late work is ignored.
